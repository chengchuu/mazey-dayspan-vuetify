...
import { ref } from 'vue'; import { DsCalendarApp,DsSchedule,useMazeyDaySpan,type CalendarEvent,type CalendarView,type EventSchedule } from 'mazey-dayspan-vuetify'
  ...
  <VApp :theme="dark?'dark':'light'">
    <VMain>
      <main class="playground">
        <header><div><h1>mazey-dayspan-vuetify</h1><p>Vue 3 calendar and recurrence playground</p></div><VSelect label="Locale" :items="['en','zh-CN']" :model-value="ds.locale.value" @update:model-value="ds.setLocale($event)" /><VSwitch v-model="dark" label="Dark mode" /></header><DsCalendarApp v-model:view="view" :date="new Date(2026,6,10)" :events="events" @event-create="upsert" @event-update="upsert" @event-remove="remove">
          <template #event="{event}">
            <strong>● {{ event.title }}</strong>
          </template>
        </DsCalendarApp><section><h2>Recurrence editor</h2><DsSchedule v-model="schedule" /><pre>{{ schedule }}</pre></section>
      </main>
    </VMain>
  </VApp>
</template>
