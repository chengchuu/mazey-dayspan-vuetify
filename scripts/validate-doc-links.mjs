...
const generatedDocs = resolve(root, 'docs')
const failures = []
...
function localTargets(source) {
  ...
  const inline = [...markdown.matchAll(/!?\[[^\]]*\]\(([^)]+)\)/g)].map((match) =>
    match[1].trim().replace(/^<|>$/g, '').split(/\s+["']/)[0],
  )
...
function validateTarget(file, target) {
  ...
  try {
    ...
    failures.push(`${relative(root, file)}: invalid URL encoding in ${target}`)
    return
  }

  const resolved = decodedPath ? resolve(dirname(file), decodedPath) : file
  ...
  if (!existsSync(resolved)) {
    failures.push(`${relative(root, file)}: missing local target ${target}`)
    return
  }
  if (!fragment || extname(resolved).toLowerCase() !== '.md') return

  let anchor
  try {
    anchor = decodeURIComponent(fragment)
  } catch {
    failures.push(`${relative(root, file)}: invalid anchor encoding in ${target}`)
    return
  }
  if (!headingAnchors(resolved).has(anchor))
    failures.push(`${relative(root, file)}: missing Markdown heading ${target}`)
}
...
for (const file of markdownFiles(generatedDocs))
  failures.push(`Generated docs directory contains Markdown source: ${relative(root, file)}`)

const sourceFiles = [resolve(root, 'README.md'), ...markdownFiles(guides)]
for (const file of sourceFiles) {
  if (!existsSync(file)) {
    failures.push(`Missing documentation source: ${relative(root, file)}`)
    continue
  }
  for (const target of localTargets(readFileSync(file, 'utf8'))) validateTarget(file, target)
}
